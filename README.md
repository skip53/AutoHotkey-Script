...
编辑U盘内的PDF等文件时，如果忘记保存，而拔下U盘，会导致之前添加的批注等丢失。该脚本对指定程序，启用自动保存。

### 4. 结束垃圾进程.ahk

某些常驻的垃圾进程：如搜狗输入法的`SogouCloud.exe`、Spotify的`SpotifyWebHelper.exe`，自动结束。

### 5. 关闭垃圾弹窗.ahk

自动关闭迅雷、QQ、搜狗等软件，弹的垃圾窗口、右下角广告等窗口（自行添加`WinTitle`）。
